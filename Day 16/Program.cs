...
namespace Day16
{
    internal class Program
    {
        ...
        private static void Part2()
        {
            ...
            sequences = new Dictionary<int, int[]>();
            int[] inputArray = new int[input.Length * 10000];
            int[] outputArray = null;
            for (int i = 0; i < inputArray.Length; i++)
            {
                inputArray[i] = int.Parse(input[i % input.Length].ToString());
            }
            ...
            for (int phase = 0; phase < 100; phase++)
            {
                outputArray = FFT(inputArray);
                inputArray = outputArray;
                Debug.WriteLine($"{phase} {sw.Elapsed}");
            }
            if (outputArray != null) Console.WriteLine(string.Join("", outputArray));
